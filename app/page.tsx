...
export default function Home() {
  ...
  const handleDragEnd = () => {
    const isDragging = isDraggingRef.current;
    if (!isDragging || !initialPositionRef.current) return;

    // CRITICAL FIX: Ensure refs are available before proceeding
    const itemRef = itemRefs[isDragging].current;
  ...
  return (
    ...
    >
      ...
      {/* FIX: Use a more reliable absolute centering method for the drop zone
        to ensure it appears correctly and is not tiny.
      */}
      <div 
        ref={dropzoneRef}
        className="
